...
2. **Attack**, fifteen modules. Quick reference (full detail below the table):
   ...
   - **Verbose Error Disclosure**: fuzzes each discovered parameter, and (when present) every scalar field of a POST/PUT/PATCH endpoint's own generated JSON body, with a single absurdly large number (`99999999999999999999999999999999999999`) - syntactically valid everywhere it lands, but a classic trigger for the runtime exceptions (overflow, a downstream `parseLong`/`parseInt` on a value meant to stay an opaque string, an ID used as an array index or allocation size, ...) that a naive error handler lets escape instead of a generic message. Deliberately not about tripping a body parser - that failure mode is much narrower and usually already handled - this is about reaching past request parsing into actual business logic. Every response (any status code, not just 5xx) is checked against a small, fixed set of unambiguous signatures: a Java stack-trace frame, a chained-exception `Caused by:` marker, a Python traceback header, or a fully-qualified `Throwable.toString()`-shaped exception name. A match is reported as `VERBOSE_ERROR_DISCLOSURE` (HIGH for an actual stack frame or `Caused by:`; MEDIUM for a bare exception class name alone). Only which signature matched is reported - the matched text itself never appears in a finding's evidence, since it may contain internal file paths or class names.
...
| `sentinel.scan.verbose-error-disclosure.enabled` | `true` | Enables/disables the Verbose Error Disclosure module |
