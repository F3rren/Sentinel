...
/**
 * Mirrors the other per-module disabled tests: disabling verbose-error-disclosure must not affect
 * the other modules.
 */
@SpringBootTest(classes = SentinelApplication.class, webEnvironment = SpringBootTest.WebEnvironment.NONE)
@TestPropertySource(properties = "sentinel.scan.verbose-error-disclosure.enabled=false")
class VerboseErrorDisclosureScannerDisabledTest {
    ...
    void onlyThisModuleIsNotRegisteredWhenDisabled() {
        assertThat(applicationContext.getBeanNamesForType(VerboseErrorDisclosureScanner.class)).isEmpty();
