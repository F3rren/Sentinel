...
/**
 * Mirrors the other per-module disabled tests: disabling data-exposure must not affect the
 * other modules.
 */
@SpringBootTest(classes = SentinelApplication.class, webEnvironment = SpringBootTest.WebEnvironment.NONE)
@TestPropertySource(properties = "sentinel.scan.data-exposure.enabled=false")
class DataExposureScannerDisabledTest {
    ...
    void onlyThisModuleIsNotRegisteredWhenDisabled() {
        assertThat(applicationContext.getBeanNamesForType(DataExposureScanner.class)).isEmpty();
